...
const TextBoxComponent = () => {
  ...
  const handleClick = async () => {
    ...
    await axios.post('http://localhost:3001/add-stock', {
      unit: inputs[0],
      size: inputs[1]
  ...
  const handleShow = async () => {
    // Save data logic here
    // After saving, clear the inputs
    await axios.get('http://localhost:3001/list-stock').then(function (response) {
  ...
  return (
    <>
      <div className = "form-container3">
        ...
        <button className = "center-button3"onClick={handleClick}>Save</button>
